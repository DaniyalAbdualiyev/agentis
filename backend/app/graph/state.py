...
class AgentState(TypedDict):
    ...
    task_id / original_task
        ...
        WHY original_task IS THE ONLY REQUIRED FIELD
        ---------------------------------------------
        LangGraph Studio builds an input form from the JSON schema of this
        TypedDict.  Fields without a default are marked `required` in the
        schema, meaning Studio forces the user to fill them in.  The only
        field a Studio user should need to provide is the task text itself
        (original_task).  All other fields are either auto-initialised by
        task_intake_node (task_id, subtask_results, retry_count, errors) or
        written by downstream nodes (execution_plan, final_output, etc.).
        Using NotRequired[] for every field except original_task removes
        them from the schema's required array, giving Studio a single-field
        input form.
    ...
    """
    # The ONLY field Studio (or any caller) must provide.
    original_task: str

    # Auto-generated by task_intake_node if not supplied by the caller.
    # NotRequired so Studio's input form doesn't demand a UUID.
    ...
    subtask_results: NotRequired[dict[str, Any]]  # subtask_id → specialist output text
    current_subtask_index: NotRequired[int]       # reserved for future per-subtask routing

    # Written by reviewer_validation; read by writer on retry
    review_feedback: NotRequired[Optional[str]]
